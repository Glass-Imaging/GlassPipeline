...
const gls::Matrix<3, 3> xyz_rgb = {{0.4124564, 0.3575761, 0.1804375},
                                   {0.2126729, 0.7151522, 0.0721750},
                                   {0.0193339, 0.1191920, 0.9503041}};

// XYZ D65 -> sRGB Transform: rgb_xyz * xyx_color -> rgb_color
const gls::Matrix<3, 3> rgb_xyz = {{3.2404542, -1.5371385, -0.4985314},
...
gls::rectangle rotate180(const gls::rectangle& rect, const gls::image<T>& image) {
    return {image.width - rect.x - rect.width, image.height - rect.y - rect.height, rect.width,
            rect.height};
}
...
gls::image<gls::rgb_pixel>::unique_ptr runPipeline(const gls::image<gls::luma_pixel_16>& rawImage,
                                                   ...
                                                   const gls::rectangle* gmb_position,
                                                   bool rotate_180);
...
gls::Vector<3> autoWhiteBalance(const gls::image<gls::luma_pixel_16>& rawImage,
                                const gls::Matrix<3, 3>& rgb_ycbcr, const gls::Vector<4>& scale_mul,
